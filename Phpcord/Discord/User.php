...
class User extends Model
{
    ...
    public function getPrivateChannel()
    {
        ...
        if (phpcord_cache()->has("pm_channel.{$this->id}")) {
            $deferred->resolve(phpcord_cache()->get("pm_channel.{$this->id}"));
        } else {
            phpcord_http()->request('users/@me/channels', 'post', ['json' => ['recipient_id' => $this->id]])->then(function ($response) use ($deferred) {
                $channel = new Channel($response, true);
                phpcord_cache()->set("pm_channel.{$this->id}", $channel);
