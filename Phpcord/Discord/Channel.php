...
use Phpcord\Stores\MessageStore;
...
class Channel extends Model
{
    ...
    protected $stores = [
        ...
        'messages' => MessageStore::class,
        //'overwrites' => OverwriteRepository::class,
    ];

    public function save()
    {
        if (!in_array($this->type, [Channel::TYPE_TEXT, Channel::TYPE_VOICE])) {
            phpcord_cache()->forever('pm_channels.' . $this->id, $this);
        }
    }
    ...
    public function getRecipientsAttribute()
    {
        ...
        if (array_key_exists('recipients', $this->attributes)) {
            foreach ((array)$this->attributes['recipients'] as $recipient) {
                $recipients->push(new User($recipient, true));
            }
        }
        return $recipients;
    }
    ...
    public function setPermissions(Model $model, ChannelPermission $permissions = null)
     * {
    ...
    {
        return discord()->guilds->get('id', $this->guild_id);
    }
    /**
    ...
    public function sendMessage($text, $tts = false, $embed = null)
    {
        ...
        phpcord_http()->request(
            ...
            function ($response) use ($deferred) {
                $message = new Message($response, true);
    ...
    public function broadcastTyping()
    {
        ...
        phpcord_http()->request("channels/{$this->id}/typing", 'post', ['headers' => ['Content-Length' => 0]])->then(
            \React\Partial\bind_right([$this, 'resolve'], $deferred),
            \React\Partial\bind_right([$this, 'reject'], $deferred)
        );
